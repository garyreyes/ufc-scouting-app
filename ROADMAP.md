...
| N9 | Replay + comparison readout. `npm run llm:replay -- --call-id=<uuid>` re-runs the entire post-model pipeline (parse → verify → apply → clamp) over a stored real payload, with zero network — a parser regression becomes a diff, not a mystery. `/scoreboard` shows all three lines (deterministic, `LLM_ASSISTED`, `LLM_ONLY`) with Brier, accuracy, and units. | not started |
