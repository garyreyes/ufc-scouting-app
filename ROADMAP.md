...
| N3 | Rumour flag retraction — the harness's first real caller, and a genuine pre-existing bug fix: nothing in `lib/rumours/` currently expires a flag once its rumour is retracted; `rumour_flags` only ever upserts and bumps `last_corroborated_at`. One card-level Lite call per run (skipped entirely when there are no open flags or no new posts — spends nothing on a quiet card) proposes `{flagId, action, supersededByUri, rationale}`. Ground-truth checks: the flag is real and still open; the superseding post is real and **strictly newer** than every existing source on that flag; the post's own text names the same fighter the flag is about (`findFighterMentionInText`, never the model's own attribution). New migration `0048_rumour_flag_retraction.sql` (`retracted_at`/`retraction_reason`/`superseded_by_post_uri` — nothing deleted, a wrong retraction is reversible by nulling a column). `fetchFlagsForFights.ts` excludes retracted flags — correctness-critical, this changes `flagPenalty()`'s input and hence `estimated_probability`. | not started |
