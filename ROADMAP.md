...
| L3 | Re-evaluate the intern's pick + bet criteria (`decideInternPick` / `decideInternBet` / `flagPenalty` / `eloAdjustment`) against real settled results and the calibration table. Depends on L2 producing data. **User direction (2026-09-10): add bounded fighter-attribute signals — age gap, reach, height, stance matchup — as new signed adjustments alongside the rumour/Elo deltas (same pattern). Also: deepen Elo (feed Sherdog full-career bouts in), fix the missing odds anchor, then calibration-tune.** | not started |
