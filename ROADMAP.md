...
| K2-followup | The Gamrot Aug 8/9 pair now surfaces as a reported skip (Aug-9 loser has a settled fight). Fully merging it needs a data-fix + confirming Louie Sutherland's real opponent — API-Sports says Henrique da Silva Lopes, Wikipedia says José Montanha. This is the last open `disputed_opponent` conflict | not started (blocked on external fact) |
