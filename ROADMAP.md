...
| M4 | Settlement cadence. `sync.yml`'s "00:00/12:00 UTC" cron actually starts 3–5.5h late live (verified against real recent run timestamps), and a Wikipedia-only result's 24h wait becomes well over 24h in practice; the Sherdog reimport was capped at 12 fighters/run in an effectively arbitrary order. New `settle.yml` (hourly, Saturday–Monday UTC, no API-Sports/Wikipedia calls — no quota cost) plus a shared `concurrency` guard across `sync.yml`/`sherdog.yml`/`settle.yml` (queues rather than cancels an overlapping run). `reimportSherdogForPendingFights.ts`'s ordering extracted into a new pure, test-first `orderSherdogIdsForReimport.ts` (newest-event-first, mutation-verified); cap raised to 30 for the weekend job via a new `--sherdog-reimport-cap=<n>` flag on `runSettlementJobs.ts`. The 24h single-source rule itself stays (owner's confirmed choice). | **built, tested, reviewed, PR open — NOT applied to production.** `CHANGES.md` Phase 78 (branch `m4-settlement-cadence`), PR #70. Reviewer pass (2026-09-17) found nothing: confirmed no API-Sports/Wikipedia call anywhere in `settle.yml`'s call graph, the concurrency group is byte-identical across all three workflows, the `eventDate` non-null assertion is safe by construction, the CLI flag falls back cleanly, and the settlement jobs are idempotent so any redundant overlap is wasted work, never wrong work. 704 tests/lint/typecheck/build all clean. |
| M5 | Sherdog auto-disambiguation via history corroboration. 10 open `low_confidence_sherdog_match` conflicts are common names (David Martínez: 20 tied candidates) or nickname-only storage (Renato Moicano = Sherdog's "Renato Carneiro"; Patrício Pitbull = "Patricio Freire"). New `historyCorroborates` (pure, mutation-verified) auto-links a candidate whose own fight history includes a bout against one of our fighter's known opponents within ±10 days of our `event_date` — verified live against the real "Patricio Freire" page. Wired into `resolveSherdogIdentityJob.ts` (new fighters) and a new `resolveOpenSherdogConflictsJob.ts` sweep (existing open conflicts). | **built, tested, reviewed, PR open — NOT applied to production.** `CHANGES.md` Phase 76 (branch `m5-sherdog-auto-disambiguation`), PR #71. Reviewer found no correctness bugs but caught two real operational issues (fixed): guaranteed duplicate Sherdog fetches within one run, and a widened race window on the sweep's final write. Live `--dry-run` against production: 10 checked, 10 would auto-resolve, 0 failed — owner go-ahead required before running for real (`npm run sherdog:resolve-open-conflicts -- --dry-run`). |
