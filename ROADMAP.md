...
| O3 (Track B) | Shadow-pick ensemble — not started. Design needs updating per the O0 spike finding: the existing whole-card shadow-picks prompt doesn't fit Groq's token budget, so this needs either a per-fight prompt variant for Groq or dropping Groq from this track entirely (re-measure before choosing). `shadow_picks` needs a `provider` column migration either way. Explicit non-goal, same as Phase N's shadow picks: never feeds real bets. | **not started.** |
