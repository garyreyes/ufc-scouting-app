...
| N4 | Conflict proposals for the `/conflicts` queue — propose, never auto-apply. One Lite call per open conflict over the candidate bundle already snapshotted in `data_conflicts.details` (no new fetch). Reduce is a **pure function**, not an LLM call: rejecting a proposal set where a fighter is both keeper and dropped is a deterministic graph check, not a judgment. Ground-truth checks reuse `checkMergeGuard`/`decideSameCardMerge` verbatim rather than re-deriving the merge safety logic. New migration `0049_conflict_resolution_proposals.sql`; UI gains a pre-selected suggestion + rationale, Accept calls the *existing* resolve actions — zero new write path. | not started |
