...
| K1-followup | Surface a K1-*skipped* cluster (blocking refs on a stale event) via `job_runs` or the `/conflicts` queue — currently console-only. Low priority: the `merged_into` filters now stop the intern/rumour jobs from creating the refs that cause a skip | **split, revisited 2026-09-19**: confirmed this has fired exactly once, ever, before the `merged_into` filter existed, and never since — the `/conflicts`-queue UI surfacing stays shelved, no evidence it's needed. But the investigation found `runScheduleSync` (`syncSchedule.ts`) was the one scheduled job never wrapped in `runWithTracking` — it wrote **zero** `job_runs` rows at all, skip or no skip. Fixed cheaply (no schema): `runScheduleSync` now takes `supabase` and returns a `ScheduleSyncSummary` (including full `mergeSkipped` detail), wrapped by a `main()` calling `runWithTracking(supabase, "sync_schedule", ...)` — the same shape every other scheduled job already uses. A K1 skip is now durable and queryable via `job_runs.summary`, without new UI. |
...
| L3-age | ⚠️ Age signal — a **peak-age curve** (27–32 costs nothing, years under 27 count half, over 32 in full; ±0.04 cap at an 8-year weighted gap), added to `decideInternPick.ts` under the unchanged 0.25 shared ceiling. **Source switched from API-Sports to Sherdog** after a live check: API-Sports' `birth_date`/`age` were null on 18/18 fighters, Sherdog carried a date on every page checked. `0042_fighter_birth_date.sql` stores a date (age computed on the card's date, never stored); `parseSherdogBirthDate`, `ageOnDate`, `birthDateFill` (refuses a date that disagrees with Sherdog's own printed age) and `ageAdjustment` all test-first. Backfill reused the existing `--refresh` import — no new script. Reasoning in `DECISIONS.md` (new). | **done** (2026-09-13) — `CHANGES.md` Phase 75. 0042 applied; dry run 132/132, 0 mismatches; live run wrote 132 birth dates, read back exactly. The next card (Silva vs. Delgado) was already inside the intern's lock, so the first stored "Age:" line lands on the following card; a read-only preview on the real card showed 9/14 fights with an age edge. |
| L3-stance | Stance-matchup signal — deferred (2026-09-10): no real, app-measured directional win-rate effect exists yet (`describeStanceMatchup.ts` stays a scoreboard-only bucket). Revisit once G3 calibration or a stance-specific accuracy breakdown shows a real direction, not a borrowed claim. | **still deferred, re-checked 2026-09-19**: `describeStanceMatchup.ts` remains display-only, no code change. Real settled-pick volume is still nowhere near enough to check for a direction — since the intern was scoped to nearest-card-only (Phase L1, 2026-09-10), only ONE card has settled (~14 picks), an order of magnitude below the app's own `SMALL_SAMPLE_THRESHOLD = 10` cards. **How to build the breakdown when the data exists**: `computeCalibrationBuckets.ts`/`computeBrierScore.ts` already take a generic `{estimatedProbability, correct}[]` — bucketing by `describeStanceMatchup()`'s string output instead of a probability band is a small, mechanical reuse, no new plumbing. Revisit once ≥10 cards have settled under the current scoping. |
...
| L4-fix | Locked intern picks are counted as `failed`, not `skippedLocked`. `isLockedError` (`generateInternPicks.ts`) reads `err.message` only when `err instanceof Error`, but a Supabase/PostgREST error is a plain object, so the lock message is never seen. Found live 2026-09-13 — the first run ever inside the intern's lock window reported "13 failed, 0 already locked". Log-only: `evaluateJobHealth` reads run status, not `summary.failed`, so no banner fires. The same run also counts `betsPlaced` before the write (reported 7 bets, wrote none). | **done** (2026-09-19). Confirmed the root cause: this codebase never calls `.throwOnError()`, so a failed `.upsert()` throws PostgREST's own plain `{message, code, details, hint}` object, never a real `Error` — `isLockedError` now reads `.message` off any object that has one, not just an `instanceof Error`. `betsPlaced++` moved inside the try block, after a confirmed successful write. New `generateInternPicks.test.ts` (there was none before) exercises the real plain-object failure shape, not a contrived one. |
| L5 | ⚠️ Method-of-victory from real Sherdog finish records, plus a 4th `FINISH` outcome. Found live 2026-09-13: the old weight-class-only rule called Fiorot vs Grasso a submission despite her 7 KO / 0 SUB / 6 DEC record, and made UFC 331's real probabilities (0.53–0.65) call DECISION for 12 of 13 fights. `predictInternMethod.ts` now uses the picked fighter's own win split + the opponent's own loss split (both required, else the old rule applies unchanged) — geometric mean on the finish-vs-decision axis, capped record shift, KO-share blended with the weight-class prior, `FINISH` for a genuinely mixed signal. `0043_predicted_method_finish.sql` widens the enum; shared with the human form. `methodBacktest.ts` (`npm run intern:method-backtest`, read-only) validated it: 74.1% exact-call accuracy vs the old rule's 69.1% on the same 68-fight both-sides population. A reviewer pass caught a straight 50/50 KO-share blend making SUBMISSION unreachable in the heavy bucket (RETROSPECTIVE.md's Phase 62 bug class, recurred) — fixed before shipping. | **done** (2026-09-13) — `CHANGES.md` Phase 76. Migration applied + read back live; backtest run live against production Sherdog history. |
...
their own `feature-planner` pass each, in order below, and ship as
independent PRs off `main` — none depends on another's merge order, so
migration numbers (and this section's own text) collide across branches and
get reconciled at merge time (an accepted, known tradeoff of parallel
sub-phases).

| # | Sub-phase | Status |
|---|---|---|
| M1 | ⚠️ Unpaged reads already causing silent data loss. `fights` (1,044 rows) had crossed PostgREST's row cap; `fetchUnpricedFights` silently returned ~998 fights instead of 1,047. Switched to `selectAllPages`/new `selectAllPagesByIds` in the odds/settlement/fighter-identity paths. Also fixed `upsertFighter`'s `.maybeSingle()` collision crash. Reviewer caught a real regression (lost `.order("name")`), fixed before shipping. | **done, merged to `main`** (2026-09-17) — `CHANGES.md` Phase 77, PR #67. |
| M2 | ⚠️ Cancelled-bout reconciliation. `processScheduleEvent.ts` only upserted bouts found on a Wikipedia card page — it never noticed one removed (Jimenez vs. Vera, cancelled for a visa issue). New `planCardReconciliation` + `applyCardReconciliation` decide missing/cancel/clear, skipped on any sign of a partial parse OR a past event date. Excluded from intern picks, odds candidates, the rumour scan, the scoreboard chalk line, the 30-day refresh queue. Card UI keeps the row, greyed. | **done, merged to `main`** (2026-09-17) — `CHANGES.md` Phase 80, PR #68. Migration `0044_cancelled_fights.sql` not yet run; cancelling Jimenez vs. Vera for real needs an explicit dry-run + owner go-ahead first. |
| M3 | ⚠️ Remembered dispute answers + fighter aliases/merge. "Keep existing" recorded nothing, so a same-card name variant ("Jose Delgado" → "Jose Miguel Delgado") reopened the identical dispute every sync — it did, twice, live. New `fighter_aliases` table + a real `merge_fighters()` DB function (ported from a one-off data-fix script), with a narrow, transaction-local pick-lock-trigger bypass for exactly that one call. Two real gaps found and fixed while writing the migration, before it ever ran: `fighter_sherdog_bouts`/`fighter_scouting_reports` are `on delete cascade` and would have silently destroyed the dropped fighter's real Sherdog history; the derived Sherdog columns needed copying alongside `sherdog_id`, not just the id itself. `isSameCardNameVariant` + `decideSameCardMerge` (pure) gate an automatic sweep (`resolveSameCardNameVariants`, wired into `sync.yml`); `/conflicts` gains a manual "same fighter" merge choice with the same hard guard (never merge two different confirmed Sherdog identities). `upsertFighter`/`upsertFight` both check the new alias table / remembered resolution before ever reopening. | **built, tested, reviewed, PR open — NOT applied to production.** `CHANGES.md` Phase 77 (branch `m3-remembered-disputes-fighter-merge`), PR #69. Reviewer pass (2026-09-17, after an earlier attempt hit a rate limit and returned nothing) found and fixed two real bugs before merge: `upsertFighter.ts`'s alias branch was overwriting the keeper's canonical name with the dropped name on every sync; `merge_fighters()` never repointed pre-existing aliases on a second-hop merge (A→B, then B→C), so the cascade delete would have silently destroyed the earlier alias. Both fixed, 733 tests/lint/typecheck/build clean. Migration renumbered `0044` → `0045` at merge time (M2's `0044_cancelled_fights.sql` merged first). Not yet run — dry-run + owner go-ahead required first (`npm run fighters:resolve-same-card-variants -- --dry-run`). |
| M4 | Settlement cadence. `sync.yml`'s "00:00/12:00 UTC" cron actually starts 3–5.5h late live (verified against real recent run timestamps), and a Wikipedia-only result's 24h wait becomes well over 24h in practice; the Sherdog reimport was capped at 12 fighters/run in an effectively arbitrary order. New `settle.yml` (hourly, Saturday–Monday UTC, no API-Sports/Wikipedia calls — no quota cost) plus a shared `concurrency` guard across `sync.yml`/`sherdog.yml`/`settle.yml` (queues rather than cancels an overlapping run). `reimportSherdogForPendingFights.ts`'s ordering extracted into a new pure, test-first `orderSherdogIdsForReimport.ts` (newest-event-first, mutation-verified); cap raised to 30 for the weekend job via a new `--sherdog-reimport-cap=<n>` flag on `runSettlementJobs.ts`. The 24h single-source rule itself stays (owner's confirmed choice). | **done, merged to `main`** (2026-09-17) — `CHANGES.md` Phase 78, PR #70. |
| M5 | Sherdog auto-disambiguation via history corroboration. 10 open `low_confidence_sherdog_match` conflicts are common names (David Martínez: 20 tied candidates) or nickname-only storage (Renato Moicano = Sherdog's "Renato Carneiro"; Patrício Pitbull = "Patricio Freire"). New `historyCorroborates` (pure, mutation-verified) auto-links a candidate whose own fight history includes a bout against one of our fighter's known opponents within ±10 days of our `event_date` — verified live against the real "Patricio Freire" page. Wired into `resolveSherdogIdentityJob.ts` (new fighters) and a new `resolveOpenSherdogConflictsJob.ts` sweep (existing open conflicts). | **done, merged to `main`** (2026-09-17) — `CHANGES.md` Phase 79, PR #71. |

---

## Phase N — Tiered map-reduce LLM, in shadow

Prompted by the user's idea to fan work out to cheap parallel LLM calls
(map) feeding a stronger call for the final decision (reduce), with
ground-truth fact-checking so hallucinations never reach the database.
Exploration found the premise didn't match this app as built — picks are
a deterministic pure function (Fork 10, `ARCHITECTURE.md`), not an LLM
call, and the only existing LLM use is rumour clustering — so the pattern
was redirected onto three real surfaces instead of applied where it was
first imagined. Full design in the approved plan
(`so-iets-plan-this-wobbly-cocke.md`, kept outside the repo per this
project's planning convention); `DECISIONS.md`'s N1 entry has the
verification-spike reasoning. Each sub-phase ships as its own
`feature-planner` pass, sequentially — N3+ depend on N2's harness, so
unlike Phase M these are not independent parallel branches.

| # | Sub-phase | Status |
|---|---|---|
| N1 | Verification spike, live, before any code — re-measured Gemini's free tier 16 days after the original F1 measurement. **The planned two-tier design (a cheap "map" model, a stronger "reduce" model) was dropped entirely**: full-Flash failed 4/6 calls with `503 UNAVAILABLE`, ran 10–20x slower, and showed no reproducible quality edge over Flash Lite on two trap-laden card-consolidation tasks. Every call now goes to one model. A follow-up dashboard check moved the real constraint from RPD (56/500, fine) to **RPM (14/15, 93%, already a live warning)** — a pre-existing production risk this spike found, not one Phase N introduced. Also confirmed: `temperature: 0` + `topK: 1` give byte-identical repeats; no dated/pinnable model id exists. | **done** (2026-09-18) — `CHANGES.md` Phase 83, `DECISIONS.md`. |
| N2 | The harness. `lib/llm.ts` → `lib/llm/`: one wrapper (`geminiClient.ts`), a generic `runMapReduce` orchestrator built to serve all three surfaces from the same code with zero network/Supabase dependency of its own, a generalized claim verifier (`verifyClaims.ts`), and a Postgres-backed budget allocator. New migration `0047_llm_call_log.sql` + `try_reserve_llm_call()` (atomic count-and-insert, advisory-locked per model id, paces calls at a 4.2s minimum interval against the 15 RPM limit N1 found production already near, not just a daily cap). `generateJson` preserved byte-for-byte — verified with a real `npx tsx runScheduledRumourJob.ts` run against production (13/13 fights via LLM, 0 fallback). 65 new tests; two correctness-critical pieces mutation-verified. | **done** (2026-09-18) — `CHANGES.md` Phase 83. Migration `0047` applied to production, verified live (function's own denial branch proven to run while writing zero rows). |
| N3 | Rumour flag retraction — the harness's first real caller, and a genuine pre-existing bug fix: nothing in `lib/rumours/` currently expires a flag once its rumour is retracted; `rumour_flags` only ever upserts and bumps `last_corroborated_at`. One card-level Lite call per run (skipped entirely when there are no open flags or no new posts — spends nothing on a quiet card) proposes `{flagId, action, supersededByUri, rationale}`. Ground-truth checks: the flag is real and still open; the superseding post is real and **strictly newer** than every existing source on that flag; the post's own text names the same fighter the flag is about (`findFighterMentionInText`, never the model's own attribution) — verified test-first, mutation-verified. New migration `0048_rumour_flag_retraction.sql` (`retracted_at`/`retraction_reason`/`superseded_by_post_uri` — nothing deleted, a wrong retraction is reversible by nulling a column). `fetchFlagsForFights.ts` excludes retracted flags — correctness-critical, this changes `flagPenalty()`'s input and hence `estimated_probability`; test-first, and the failing test reproduced the real pre-existing bug before the fix landed. **Deliberately deferred**: `parseClusterResponse.ts`'s hand-rolled checks were not refactored into `ClaimCheck`s (the plan's original stretch goal) — the working, already-tested clustering path wasn't worth the regression risk for a consistency-only change with no new capability; `retractionChecks.ts` already proves the pattern generalizes for N4. | **done** (2026-09-18) — `CHANGES.md` Phase 84. Migration `0048` applied to production, verified live (constraint genuinely rejects an inconsistent write; real job run against production made exactly one retraction-pass call, 0/0 retracted — no contradicting evidence found, the correct outcome). |
| N4 | Conflict proposals for `/conflicts` — propose, never auto-apply. **Scoped to `low_confidence_sherdog_match` only** (not `disputed_opponent`): the plan's own worked example (Renato Moicano = Sherdog's "Renato Carneiro") and its "10/10 already auto-resolved by the heuristic" framing are both about Sherdog identity matching, which links ONE fighter row to an external id — a lower-stakes question than `disputed_opponent`'s optional `merge_fighters()` path, which destroys rows permanently and was kept deliberately out of scope. One Lite call per open conflict, over the candidate list already snapshotted in `details` — genuinely decomposes (unlike N3), so this is a real map step, not a single pseudo-unit. Reduce is a **pure function**, `reconcileSherdogProposals.ts`: `fighters.sherdog_id` is unique, so if two different conflicts each propose the same id, only the first is kept — test-first, mutation-verified. Ground-truth checks (test-first, mutation-verified): a cited candidate id must be real for *that conflict's own* list, never any candidate list in scope. `MappedUnit` gained a `callLogId` for real per-decision traceability (N2's `runMapReduce.ts`, tests updated). New migration `0049_conflict_resolution_proposals.sql`; runs as the last step of the existing `sherdog.yml` job, after the heuristic sweep, so it only ever spends a call on the real residual. UI (`LowConfidenceSherdogMatchCard.tsx`) pre-selects the suggestion and shows the rationale, labeled "advisory only, not applied"; Accept calls the *existing* `resolveSherdogMatchAction` — zero new write path. `low_confidence_fighter_match` (a second, less-examined conflict kind) deliberately deferred — same architecture would extend to it later. | **done** (2026-09-18) — `CHANGES.md` Phase 85. Migration `0049` applied to production, verified live: a synthetic test conflict, run through the real pipeline (real Gemini call, real write), correctly picked the exact-name-match candidate over a decoy — then fully cleaned up, zero residue left in production. |
| N5 | Per-signal breakdown on `picks` — a pure refactor, prerequisite for N8's comparison being interpretable at all. `decideInternPick` already computes every delta (rumour/Elo/size/age); this returns them structured (`signals jsonb`, new migration `0050_pick_signals.sql`) instead of only concatenating them into the `reasoning` prose column. Gate: a characterization test asserting bit-identical probabilities across a fixture corpus before and after. | **done** (2026-09-18) — `CHANGES.md` Phase 86. Migration `0050` applied to production, verified live: 14-fixture characterization snapshot stayed bit-identical across the refactor; real `intern:scheduled-job` run wrote populated `signals` on 12/12 production picks. |
| N6 | Brier scoring + the pre-registered stopping rule. New `computeBrierScore.ts` (there is no proper scoring rule anywhere in the repo today) wired into `/scoreboard`. The promotion criterion (number of cards, winning margin, "no clear winner → deterministic stays") gets written into `DECISIONS.md` **before** N8 writes its first shadow pick, on purpose — deciding it after seeing the numbers would mean picking whichever threshold agrees with what's already hoped. Also records the standing rule: **never backtest the LLM scout over settled historical fights** — an LLM asked about a named past fight likely has the real result in training data, and a historical backtest would return a fake accuracy number nothing in lint/tests/build could catch. Forward shadow only. | **done** (2026-09-18) — `CHANGES.md` Phase 87, `DECISIONS.md`. Test-first, mutation-verified (8/8 tests, inverting the scoring formula reproduced exactly 4 failures). Promotion rule: 10-card minimum (reusing `SMALL_SAMPLE_THRESHOLD`), 0.02 Brier-score margin, ties/worse resolve to "deterministic stays." |
| N7 | Scouting dossiers — the map step. One Lite call per **fighter** (not per fight), content-addressed: `odds_snapshots` is immutable and Elo only changes on settlement, so a fighter's dossier is cached on a hash of its exact input bundle and only recomputed when something in it actually changed. New migration `0051_fighter_scouting_dossiers.sql`. | **done** (2026-09-18) — `CHANGES.md` Phase 88. Found and fixed a real pacing bug in the shared `runMapReduce.ts` harness (no caller had ever paced calls, despite the architecture doc saying so) — the first real 24-unit fan-out this harness has had; 20/24 denied before the fix, 20/20 written after. Migration `0051` applied to production, cache confirmed working live (0 calls on a clean re-run). |
| N8 | Shadow picks — the reduce step. One call per card, only when ≥1 dossier changed. The model never emits a probability directly — it emits bounded signed deltas per named signal, applied through the *existing* `applyProbabilityDelta`/`MAX_TOTAL_ADJUSTMENT` clamp, so the comparison is apples-to-apples (better deltas than the hand-set constants, not "can it do arithmetic"). The same call also carries the model's own unconstrained probability — two shadow lines, `LLM_ASSISTED` and `LLM_ONLY`, for the price of one call, answering both "assist the rule" and "replace it" without extra quota. New migration `0052_shadow_picks.sql`, a **separate table**, never a third `picks.author` — settlement computed live, reusing the existing scoring functions, never stored. | **done** (2026-09-18) — `CHANGES.md` Phases 89-90. "Whole card as one map unit" trick makes `runMapReduce.ts`'s per-unit call loop produce exactly one call per card. Migration `0052` applied to production. First live `workflow_dispatch` run (Phase 90) found a 100%-reproducible prompt bug — reach/height rendered as `"193cm"`, echoed back as a string, failing the strict numeric-restatement check on every fight — fixed and reverified live: 12/12 fights, 24 real rows, zero drops. |
| N9 | Replay + comparison readout. `npm run llm:replay -- --call-id=<uuid>` re-runs the entire post-model pipeline (parse → verify → apply → clamp) over a stored real payload, with zero network — a parser regression becomes a diff, not a mystery. `/scoreboard` shows all three lines (deterministic, `LLM_ASSISTED`, `LLM_ONLY`) with Brier, accuracy, and units. | **done** (2026-09-19) — `CHANGES.md` Phase 94. `selectLatestBeforeLock.ts`/`scoreShadowLines.ts` (test-first, correctness-critical) read `shadow_picks` at the latest-row-strictly-before-lock instant; `getScoreboardData` restricts the deterministic comparison line to the exact same scored population, not the intern's whole settled history. `ShadowComparisonTable` renders below `PickHistoryTable`. Replay CLI locates a call's fights from its own raw output, not `shadow_picks` (the one call most worth replaying wrote zero rows there), with an `--event-id=` fallback. Reviewer pass found and fixed one real bug: the fallback could silently override a correctly-extracted event id. |
...
| O3 (Track B) | Shadow-pick ensemble. A live spike confirmed a per-fight Groq prompt fits its 8000 TPM budget with ~4x headroom (~2100 tokens measured), reusing `buildShadowPicksPrompt.ts` unchanged with a length-1 array — Groq makes one call per fight (~11-14/card) via new `generateShadowPicksGroq.ts`, a separate job from Gemini's on purpose. New migration `0061_shadow_picks_provider.sql`. Found and fixed a real pre-existing bug while planning: `selectLatestBeforeLock.ts` deduped by `fightId:line` only, so a second provider's rows for the same fight/line would have silently collided and vanished from scoring — fixed test-first, dedup key now includes `provider`. `/scoreboard`'s comparison table shows a row pair per provider present. Explicit non-goal, same as Phase N's shadow picks: never feeds real bets. | **done** (2026-09-20) — `CHANGES.md` Phase 98. Migration `0061` applied to production, verified via `supabase migration list --linked`. `llm:replay` stays Gemini-scoped this pass. |
