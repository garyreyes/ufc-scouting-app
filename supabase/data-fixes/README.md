...
| `2026-09-10_merge-gamrot-salkilld-date-split.sql` | One "Gamrot vs. Salkilld" card as two `events` rows on adjacent dates (2026-08-08 Wikipedia / 08-09 API-Sports); K2 detects the ±1-day split but skips it (Aug-9 loser carried stale winners). Merged manually: 12 stale fights + 24 double-counted Elo rows deleted, last open `disputed_opponent` resolved (Sutherland fought José Montanha) |
