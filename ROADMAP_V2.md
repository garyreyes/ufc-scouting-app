...
| **Q4** | UI: record a slip (archetype, legs, taken prices, stake, book), list open slips, settle / cash-out. | Judgment | **done 2026-09-22** — `/betting` (SlipForm/SlipList) plus `settleBetSlips.ts`, the automated won/lost/void job Q4's "settle" half actually needs (0064's triggers block a client from ever writing those); wired into `runSettlementJobsOnce.ts`, riding `settle.yml`'s cron |
| **Q5** | Per-archetype units/ROI line + bankroll curve, and the INTERN-vs-owner head-to-head the tickets already support — Elliott, Bukauskas, Hooker and Rahiki all overlap fights INTERN priced. | Judgment | **done 2026-09-22** — new `/betting/report` page (`ArchetypeRoiBoard`/`BankrollCurveChart`/`HeadToHeadBoard`, `features/betting/reportApi.ts`). Head-to-head restricted to single-leg MONEYLINE slips only (a parlay's slip-level P&L can't be attributed to one fight — `DECISIONS.md`), verified against real seed data: Bukauskas/Hooker pair cleanly, the parlay'd Elliott leg and Rahiki's method bet are correctly excluded. `reviewer` pass caught a real hover-hit-rect spacing bug in the new chart before ship. 1253/1253 tests, lint clean, build clean |
